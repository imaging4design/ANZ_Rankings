...
class Records_con extends CI_Controller {
	...
	public function index()
	{
		...
		if($query = $this->records_model->get_default())
		{
	...
	public function get_records()
	{
		...
		{
			if($this->input->post('in_out') == 'out')
			{
				// Get (Outdoor) records data from database
			...
			{
				// Get (Indoor) records data from database
				if($query = $this->records_model->get_indoor_records())
				{
					$data['get_records'] = $query;
				}
			}
