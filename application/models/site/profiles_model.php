...
class Profiles_Model extends CI_Model
{
	...
	public function __construct()
	{
		...
		else
		{
			...
			if(in_array($this->input->post('eventID'), $this->config->item('multi_events')))
				{
					$this->year = "YEAR(resMulti.date) = ". $this->input->post('year'); // Use this if a multi event
				}
				else
				{
					$this->year = "YEAR(results.date) = ". $this->input->post('year'); // Use this if an individual event
				}
		}
	...
	public function athlete()
	{
		...
		$this->db->where('athletes.athleteID', $this->athleteID);
	...
	public function athlete_data()
	{
		...
		$this->db->where('results.eventID', $this->input->post('eventID'));
	...
	public function athlete_multi_data()
	{
		...
		$this->db->where('athletes.athleteID', $this->uri->segment(4));
		$this->db->where('resMulti.eventID', $this->input->post('eventID'));
		$this->db->where($this->year);
